const fetchBadges = require("../fetchers/hackerrank");

const badges = (username, cb) => {
  const getBadges = (badgeList) => {
    const getBadge = (title, stars, totalStars, position) => `
  <svg viewBox="0 0 91.66667 100" width="91.66667" height="100" x="${
    ...
    <image href="${process.env.IMAGE_HOST}/resources/svgs/${title
      .replaceAll(" ", "-")
  ...
  fetchBadges(username, (result) => {
    if (result) {
      const badgeList = result.models.filter((b) => b.stars > 0);
      cb(getBadges(badgeList));
    } else cb(result);
  });
};
